...
async def playlist(client, message):
    ...
    msg = "🦹🏻 ** يعمل الآن ** قيد التشغيل {}".format(message.chat.title)
    msg += "\n\n• " + now_playing
    msg += "\n• Req By " + by
    temp.pop(0)
    if temp:
        msg += "\n\n"
        msg += "🎸 ** أغنية في قائمة الانتظار:**"
        for song in temp:
            name = song[0]
...
async def settings(client, message):
    ...
    if stats:
        ...
        await message.reply(
            "🎤 ** الدردشة الصوتية غير موجودة**\n\n» يرجى تشغيل الدردشة الصوتية أولاً"
        )
...
async def music_onoff(_, message):
    ...
    if status in ("ON", "on", "On"):
        ...
        if not message.chat.id in DISABLED_GROUPS:
            await lel.edit("» ** مشغل الموسيقى قيد التشغيل بالفعل.**")
            return
        DISABLED_GROUPS.remove(message.chat.id)
        await lel.edit(f"✅ **مشغل الموسيقى قيد التشغيل**\n\n💬 `{message.chat.id}`")

    elif status in ("OFF", "off", "Off"):
        lel = await message.reply("`يتم المعالجة...`")

        if message.chat.id in DISABLED_GROUPS:
            await lel.edit("» **مشغل الموسيقى متوقف بالفعل.**")
            return
        DISABLED_GROUPS.append(message.chat.id)
        await lel.edit(f"✅ **تم إيقاف تشغيل مشغل الموسيقى**\n\n💬 `{message.chat.id}`")
    else:
        await message.reply_text(
            "**• استعمل:**\n\n `/music on` & `/music off`"
        )
...
async def p_cb(b, cb):
    ...
    if type_ == "playlist":
        ...
        now_playing = temp[0][0]
        by = temp[0][1].mention(style="md")
        msg = "💡 **الأن يعمل** on {}".format(cb.message.chat.title)
        msg += "\n\n• " + now_playing
        msg += "\n• Req by " + by
        temp.pop(0)
        if temp:
...
async def m_cb(b, cb):
    ...
    global que
    if (
        cb.message.chat.title.startswith("قتاه الاغاني: ")
